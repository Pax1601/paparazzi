      ...
      for (ii = ilo; ii + 1 < ihi - 1; ii++) {
        for (nzcount = ihi - 1; nzcount + 1 > ii + 2; nzcount--) {
          ...
          for (j = ii + 1; j + 1 <= ihi; j++) {
            ctoc = absxk * b_A_data[(nzcount + b_A_size[0] * j) - 1].re +
              (atmp.re * b_A_data[nzcount + b_A_size[0] * j].re - atmp.im *
               b_A_data[nzcount + b_A_size[0] * j].im);
            stemp_im = absxk * b_A_data[(nzcount + b_A_size[0] * j) - 1].im +
            ...
            b_A_data[(nzcount + b_A_size[0] * j) - 1].re = ctoc;
            b_A_data[(nzcount + b_A_size[0] * j) - 1].im = stemp_im;
          }

          atmp.re = -atmp.re;
          atmp.im = -atmp.im;
          for (i = ilo; i + 1 <= ihi; i++) {
            ctoc = absxk * b_A_data[i + b_A_size[0] * nzcount].re + (atmp.re *
              b_A_data[i + b_A_size[0] * (nzcount - 1)].re - atmp.im *
              b_A_data[i + b_A_size[0] * (nzcount - 1)].im);
            stemp_im = absxk * b_A_data[i + b_A_size[0] * nzcount].im + (atmp.re
            ...
            b_A_data[i + b_A_size[0] * nzcount].re = ctoc;
            b_A_data[i + b_A_size[0] * nzcount].im = stemp_im;
          }
        }
      }
    }

    eml_matlab_zhgeqz(b_A_data, b_A_size, ilo + 1, ihi, info, alpha1_data,
...
void roots(const double c[5], creal_T r_data[], int r_size[1])
{
  ...
  if (k1 < k2) {
    ...
    if (companDim < 1) {
      ...
      k1 = eiga_size[0];
      for (j = 0; j < k1; j++) {
        eiga_data_re = eiga_data[j].re;
        eiga_data_im = eiga_data[j].im;
        if (beta1_data[j].im == 0.0) {
          if (eiga_data[j].im == 0.0) {
            eiga_data[j].re /= beta1_data[j].re;
            eiga_data[j].im = 0.0;
          } else if (eiga_data[j].re == 0.0) {
            eiga_data[j].re = 0.0;
            eiga_data[j].im = eiga_data_im / beta1_data[j].re;
          } else {
            eiga_data[j].re /= beta1_data[j].re;
            eiga_data[j].im = eiga_data_im / beta1_data[j].re;
          }
        } else if (beta1_data[j].re == 0.0) {
          if (eiga_data[j].re == 0.0) {
            eiga_data[j].re = eiga_data[j].im / beta1_data[j].im;
            eiga_data[j].im = 0.0;
          } else if (eiga_data[j].im == 0.0) {
            eiga_data[j].re = 0.0;
            eiga_data[j].im = -(eiga_data_re / beta1_data[j].im);
          } else {
            eiga_data[j].re = eiga_data[j].im / beta1_data[j].im;
            eiga_data[j].im = -(eiga_data_re / beta1_data[j].im);
          }
        } else {
          brm = fabs(beta1_data[j].re);
          bim = fabs(beta1_data[j].im);
          if (brm > bim) {
            bim = beta1_data[j].im / beta1_data[j].re;
            d = beta1_data[j].re + bim * beta1_data[j].im;
            eiga_data[j].re = (eiga_data[j].re + bim * eiga_data[j].im) / d;
            eiga_data[j].im = (eiga_data_im - bim * eiga_data_re) / d;
          } else if (bim == brm) {
            if (beta1_data[j].re > 0.0) {
              bim = 0.5;
            } else {
              bim = -0.5;
            }
            ...
            eiga_data[j].re = (eiga_data[j].re * bim + eiga_data[j].im * d) /
              brm;
            eiga_data[j].im = (eiga_data_im * bim - eiga_data_re * d) / brm;
          } else {
            bim = beta1_data[j].re / beta1_data[j].im;
            d = beta1_data[j].im + bim * beta1_data[j].re;
            eiga_data[j].re = (bim * eiga_data[j].re + eiga_data[j].im) / d;
            eiga_data[j].im = (bim * eiga_data_im - eiga_data_re) / d;
          }
        }
      }
