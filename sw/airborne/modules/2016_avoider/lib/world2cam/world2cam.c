...
void world2cam(double x, double y, double z, double m[2])
{
  ...
  int idx;
  int ii;
  double poly_coef_tmp[5];
  static const double poly_coef[5] = { 6.3102939082767464E-8,
    -3.65747202460342E-6, 0.003504112195699, 0.0, -108.5684719838918 };

  int rhoTmp_size[1];
  creal_T rhoTmp_data[4];
  boolean_T x_data[4];
  int ii_data[4];
  boolean_T exitg2;
  ...
  poly_coef_tmp[3] = 0.0 - z / sqrt(M_idx_0 * M_idx_0 + M_idx_1 * M_idx_1);
  ...
  m[0] = (rhoTmp_data[0].re * 0.998944414505075 + res_data[0].re *
          0.00046182179991509213) + 138.36187621095;
  m[1] = (rhoTmp_data[0].re * -0.004376565126497 + res_data[0].re) +
     122.07952695775229;
}
